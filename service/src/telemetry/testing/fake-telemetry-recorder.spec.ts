...
describe('FakeTelemetryRecorder', () => {
  it('records every telemetry call it receives', async () => {
    ...
    expect(recorder.retryOutcomes).toEqual([
      { stage: 'extraction', budget: 'validation', outcome: 'succeeded', attempts: 1 },
    ]);
    expect(recorder.tokenUsages).toEqual([
      { call: 'extract', candidate_issue_number: null, input_tokens: 10, output_tokens: 5 },
    ]);
    expect(recorder.stageLatencies).toEqual([
      { stage: 'extraction', duration_ms: 42, candidate_issue_number: null },
    ]);
    expect(recorder.outcomes).toEqual(['issue_created']);
    expect(recorder.duplicateVerdicts).toEqual(['clear_duplicate']);
    expect(recorder.duplicateJudgmentSkips).toEqual([
      { reportHash: 'abc123', candidateIssueNumber: 9 },
    ]);
    expect(recorder.logs).toEqual([
      { reportHash: 'abc123', stage: 'extraction', fields: { duration_ms: 5 } },
    ]);
