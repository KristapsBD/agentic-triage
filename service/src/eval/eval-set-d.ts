...
function checkComponentsInclude(expected: Component): Check {
  return (body) => {
    ...
    return actual.includes(expected)
      ? []
      : [
          `expected components to include ${JSON.stringify(expected)}, got ${JSON.stringify(actual)}`,
        ];
  };
}
...
function postCheckBodyContains(
  ...
  return async (body) => {
    ...
    try {
      ...
      return (issue.body ?? '').includes(needle)
        ? []
        : [`issue #${number} body did not contain ${JSON.stringify(needle)} verbatim`];
    } catch (err) {
      return [`could not fetch issue #${number} to verify: ${(err as Error).message}`];
    }
  };
...
const CASES: Case[] = [
  ...
  },
  {
    ...
    check: all(
      checkReportType('bug'),
      checkOutcome('issue_created'),
      checkComponentsInclude('frontend'),
    ),
  ...
  },
  {
    ...
    check: all(
      checkOutcome('review_flagged'),
      checkDuplicateTierNot('not_a_duplicate'),
      checkComponentsMin(2),
    ),
  ...
  },
  {
    ...
    check: all(
      checkReportType('bug'),
      checkSeverityIn(['critical', 'high']),
      checkComponentsInclude('backend'),
    ),
